    ...
    pub fn generate_report_with_sensors(
        ...
        if let Some((node_idx, spike)) = worst_spike {
            ...
            insights.push(format!(
                "<li><b>Highest PM2.5:</b> {:.1} µg/m³ (AQI {}, {}) at {} — {}</li>",
        ...
        if let Some(front) = analysis.fronts.first() {
            insights.push(format!(
                "<li><b>Strongest front:</b> {} → {} at {:.0} km/h {} (correlation {:.0}%, lag {}h)</li>",
                html_escape(&front.from_city),
                html_escape(&front.to_city),
                front.speed_kmh,
        ...
        if let Some(w) = wind {
            if let (Some(speed), Some(dir)) = (w.wind_speed_10m, w.direction_label()) {
                let condition = if speed < 5.0 {
                    "calm — pollutants tend to accumulate locally"
                } else if speed < 15.0 {
                    "moderate — pollution disperses gradually"
                } else {
                    "strong — good ventilation, rapid dispersal"
                };
                insights.push(format!(
                    "<li><b>Wind:</b> {:.1} km/h {} — {}</li>",
                    speed, dir, condition,
                ));
            }
        }
